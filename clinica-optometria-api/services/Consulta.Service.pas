...
type
  ...
  public
    ...
    function Criar(APacienteId: Integer; const AProcedimento, AProfissional: string;
      AAgendamentoId: Integer; AData: TDateTime = 0; const AStatus: string = '';
      const AOrigem: string = ''; AUsuarioId: Integer = 0;
      const AObservacoes: string = ''): Integer;
...
function CampoConsultaExiste(const ACampo: string): Boolean;
var
  LQuery: iQuery;
begin
  ...
  LQuery.Add('WHERE TRIM(RDB$RELATION_NAME) = ''CONSULTAS'' ');
...
procedure GarantirCampoConsulta(const ACampo, ATipo: string);
var
  LIndiceConexao: Integer;
  LConn: TFDConnection;
  LQuery: TFDQuery;
begin
  if CampoConsultaExiste(ACampo) then
    Exit;
  try
    ...
    try
      ...
      try
        ...
        try
          LQuery.SQL.Text := 'ALTER TABLE CONSULTAS ADD ' + ACampo + ' ' + ATipo;
          LQuery.ExecSQL;
          LConn.Commit;
        except
...
class procedure TConsultaService.Inicializar;
var
  LService: TConsultaService;
begin
  LService := TConsultaService.Create;
  try
    LService.GarantirTabelas;
  finally
    LService.Free;
  end;
end;
...
begin
  ...
  GarantirCampoConsulta('CON_ORIGEM', 'VARCHAR(30)');
  GarantirCampoConsulta('CON_CRIADO_EM', 'TIMESTAMP');
  GarantirCampoConsulta('CON_CRIADO_POR', 'INTEGER');
  GarantirCampoConsulta('CON_OBSERVACOES', 'VARCHAR(500)');
...
begin
  ...
  LQuery.Add('C.CON_ORIGEM AS ORIGEM, C.CON_CRIADO_EM AS CRIADO_EM, C.CON_OBSERVACOES AS OBSERVACOES ');
...
begin
  ...
  LQuery.Add('C.CON_DATA AS DATA, C.CON_FINALIZADA_EM AS FINALIZADA_EM, ');
  LQuery.Add('C.CON_ORIGEM AS ORIGEM, C.CON_CRIADO_EM AS CRIADO_EM, C.CON_CRIADO_POR AS CRIADO_POR, C.CON_OBSERVACOES AS OBSERVACOES, ');
  LQuery.Add('P.PAC_NOME AS PACIENTE_NOME, ');
...
function TConsultaService.Criar(APacienteId: Integer; const AProcedimento,
  AProfissional: string; AAgendamentoId: Integer; AData: TDateTime;
  const AStatus: string; const AOrigem: string; AUsuarioId: Integer;
  const AObservacoes: string): Integer;
var
  LAtendimento: TAtendimentoService;
  LStatusAtendimento: string;
  LProfissional: string;
  LProcedimento: string;
  LStatus: string;
  LOrigem: string;
  LData: TDateTime;
  LFinalizadaEm: TDateTime;
  LTemFinalizadaEm: Boolean;
  LIndiceConexao: Integer;
  LConn: TFDConnection;
  LQuery: TFDQuery;
begin
  ...
  begin
    ...
    try
      try
        Result := LAtendimento.Iniciar(AAgendamentoId, LStatusAtendimento);
      except
  ...
  LProfissional := Trim(AProfissional);
  if LProfissional = '' then
    LProfissional := 'Profissional';

  LProcedimento := Trim(AProcedimento);
  ...
  if LStatus = '' then
    LStatus := STATUS_REALIZADA;

  if not StatusPermitido(LStatus) then
    raise EConsultaValidacao.Create('Status de consulta invalido');

  LTemFinalizadaEm := (LStatus = STATUS_REALIZADA) or (LStatus = STATUS_ATENDIDA);
  if LTemFinalizadaEm then
  begin
    if AData > 0 then
      LFinalizadaEm := AData
    else
    ...
    try
      ...
      try
        ...
        LQuery.SQL.Text :=
          ...
          'CON_PROCEDIMENTO, CON_STATUS, CON_DATA, CON_FINALIZADA_EM, ' +
          'CON_ORIGEM, CON_CRIADO_EM, CON_CRIADO_POR, CON_OBSERVACOES) ' +
          'VALUES (:CON_ID, :CON_PACIENTE_ID, :CON_AGENDAMENTO_ID, :CON_PROFISSIONAL, ' +
          ':CON_PROCEDIMENTO, :CON_STATUS, :CON_DATA, :CON_FINALIZADA_EM, ' +
          ':CON_ORIGEM, :CON_CRIADO_EM, :CON_CRIADO_POR, :CON_OBSERVACOES)';

        LQuery.ParamByName('CON_ID').AsInteger := Result;
        LQuery.ParamByName('CON_PACIENTE_ID').AsInteger := APacienteId;
        LQuery.ParamByName('CON_AGENDAMENTO_ID').Clear;
        LQuery.ParamByName('CON_PROFISSIONAL').AsString := LProfissional;
        LQuery.ParamByName('CON_PROCEDIMENTO').AsString := LProcedimento;
        LQuery.ParamByName('CON_STATUS').AsString := LStatus;
        LQuery.ParamByName('CON_DATA').AsDateTime := LData;

        if LTemFinalizadaEm then
          LQuery.ParamByName('CON_FINALIZADA_EM').AsDateTime := LFinalizadaEm
        else
          LQuery.ParamByName('CON_FINALIZADA_EM').Clear;

        LQuery.ParamByName('CON_ORIGEM').AsString := LOrigem;
        LQuery.ParamByName('CON_CRIADO_EM').AsDateTime := Now;

        if AUsuarioId > 0 then
          LQuery.ParamByName('CON_CRIADO_POR').AsInteger := AUsuarioId
        else
          LQuery.ParamByName('CON_CRIADO_POR').Clear;

        if Trim(AObservacoes) <> '' then
          LQuery.ParamByName('CON_OBSERVACOES').AsString := Trim(AObservacoes)
        else
          LQuery.ParamByName('CON_OBSERVACOES').Clear;
