...
begin
  ...
  LQuery.Add('COALESCE(R.RET_TEM_RETORNO, 1) AS TEM_RETORNO, ');
  LQuery.Add('COALESCE(R.RET_TEM_NOVA_CONSULTA, 0) AS TEM_NOVA_CONSULTA, ');
  LQuery.Add('R.RET_NOVA_CONSULTA_DATA AS NOVA_CONSULTA_DATA, ');
  LQuery.Add('R.RET_NOVA_CONSULTA_MOTIVO AS NOVA_CONSULTA_MOTIVO, ');
  LQuery.Add('R.RET_NOVA_CONSULTA_OBSERVACAO AS NOVA_CONSULTA_OBSERVACAO, ');
  ...
  LQuery.Add('CASE WHEN R.RET_NOVA_CONSULTA_DATA < CURRENT_DATE THEN ''vencido'' ');
  LQuery.Add('WHEN R.RET_NOVA_CONSULTA_DATA <= DATEADD(30 DAY TO CURRENT_DATE) THEN ''proximo'' ');
  LQuery.Add('ELSE ''programado'' END AS NOVA_CONSULTA_STATUS ');
  ...
  LQuery.Add('WHERE (R.RET_DATA IS NOT NULL AND (R.RET_TEM_RETORNO = 1 OR R.RET_TEM_RETORNO IS NULL)) ');
  LQuery.Add('   OR (R.RET_NOVA_CONSULTA_DATA IS NOT NULL AND R.RET_TEM_NOVA_CONSULTA = 1) ');
  LQuery.Add('ORDER BY COALESCE(R.RET_DATA, R.RET_NOVA_CONSULTA_DATA), R.RET_ID');
  LQuery.Open;
  Result := TDatasetJsonUtils.QueryToJSONArray(LQuery.DataSet);
end;
