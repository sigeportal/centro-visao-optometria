...
begin
  ...
  if not Assigned(ADados) then
    Exit;
  LValue := Trim(ADados.GetValue<string>(AName, ''));
  if LValue = '' then
    Exit;
  ...
  try
    try
      ...
      begin
        ...
        TLogger.Error(Format('[%s] %s', [LCorrelationId, 'ConsultaController.ObterFicha']), E);
...
begin
  ...
  try
    try
      ...
      begin
        LCorrelationId := ObterCorrelationId(Req);
        TLogger.Error(Format('[%s] %s', [LCorrelationId, 'ConsultaController.ObterSecaoFicha']), E);
        Res.Send<TJSONObject>(TResponseUtils.InternalError('Ocorreu um erro interno ao processar a solicitacao', LCorrelationId))
          .Status(THTTPStatus.InternalServerError);
      end;
    end;
...
begin
  ...
  try
    try
      ...
      begin
        LCorrelationId := ObterCorrelationId(Req);
        TLogger.Error(Format('[%s] %s', [LCorrelationId, 'ConsultaController.SalvarSecaoFicha']), E);
        Res.Send<TJSONObject>(TResponseUtils.InternalError('Ocorreu um erro interno ao processar a solicitacao', LCorrelationId))
          .Status(THTTPStatus.InternalServerError);
      end;
    end;
...
begin
  ...
  try
    try
      ...
      begin
        LCorrelationId := ObterCorrelationId(Req);
        TLogger.Error(Format('[%s] %s', [LCorrelationId, 'ConsultaController.ObterCompletudeFicha']), E);
        Res.Send<TJSONObject>(TResponseUtils.InternalError('Ocorreu um erro interno ao processar a solicitacao', LCorrelationId))
          .Status(THTTPStatus.InternalServerError);
      end;
    end;
...
begin
  ...
  try
    try
      ...
      begin
        LCorrelationId := ObterCorrelationId(Req);
        TLogger.Error(Format('[%s] %s', [LCorrelationId, 'ConsultaController.Listar']), E);
        Res.Send<TJSONObject>(TResponseUtils.InternalError('Ocorreu um erro interno ao processar a solicitacao', LCorrelationId)).Status(THTTPStatus.InternalServerError);
      end;
    end;
  finally
    Service.Free;
...
begin
  ...
  try
    try
      ...
      begin
        LCorrelationId := ObterCorrelationId(Req);
        TLogger.Error(Format('[%s] %s', [LCorrelationId, 'ConsultaController.ObterPorId']), E);
        Res.Send<TJSONObject>(TResponseUtils.InternalError('Ocorreu um erro interno ao processar a solicitacao', LCorrelationId)).Status(THTTPStatus.InternalServerError);
      end;
    end;
  finally
...
begin
  ...
  try
    try
      ...
      LPacienteId := LBody.GetValue<Integer>('paciente_id', 0);
      LProcedimento := LBody.GetValue<string>('procedimento', 'Consulta');
      LProfissional := LBody.GetValue<string>('profissional', '');
      LAgendamentoId := LBody.GetValue<Integer>('agendamento_id', 0);
      LData := JsonDate(LBody, 'data');
      LStatus := LBody.GetValue<string>('status', '');
      LObservacoes := LBody.GetValue<string>('observacoes', '');
      LOrigem := LBody.GetValue<string>('origem', '');
      ...
      // Se agendamento_id <= 0 (ou origem = 'retroativa'):
      if (LAgendamentoId <= 0) or SameText(LOrigem, 'retroativa') then
      begin
        if LData > 0 then
          LDataStr := FormatDateTime('yyyy-mm-dd hh:nn:ss', LData)
        else
          LDataStr := FormatDateTime('yyyy-mm-dd hh:nn:ss', Now);

        if LProfissional = '' then
          LProfissional := 'Profissional';

        TAuditoriaService.Registrar(
          Req,
          'INSERT',
          'CONSULTAS',
          LId,
          Format('Registro de consulta retroativa do paciente %d ref data %s pelo profissional %s',
            [LPacienteId, LDataStr, LProfissional])
        );
      end;
      ...
      begin
        LCorrelationId := ObterCorrelationId(Req);
        TLogger.Error(Format('[%s] %s', [LCorrelationId, 'ConsultaController.Criar']), E);
        Res.Send<TJSONObject>(TResponseUtils.InternalError('Ocorreu um erro interno ao processar a solicitacao', LCorrelationId)).Status(THTTPStatus.InternalServerError);
      end;
    end;
  finally
...
begin
  ...
  try
    try
      ...
      begin
        LCorrelationId := ObterCorrelationId(Req);
        TLogger.Error(Format('[%s] %s', [LCorrelationId, 'ConsultaController.Atualizar']), E);
        Res.Send<TJSONObject>(TResponseUtils.InternalError('Ocorreu um erro interno ao processar a solicitacao', LCorrelationId)).Status(THTTPStatus.InternalServerError);
      end;
    end;
  finally
...
begin
  ...
  try
    try
      ...
      begin
        LCorrelationId := ObterCorrelationId(Req);
        TLogger.Error(Format('[%s] %s', [LCorrelationId, 'ConsultaController.Finalizar']), E);
        Res.Send<TJSONObject>(TResponseUtils.InternalError('Ocorreu um erro interno ao processar a solicitacao', LCorrelationId)).Status(THTTPStatus.InternalServerError);
      end;
    end;
  finally
...
begin
  ...
  try
    try
      ...
      begin
        LCorrelationId := ObterCorrelationId(Req);
        TLogger.Error(Format('[%s] %s', [LCorrelationId, 'ConsultaController.ListarAnamneses']), E);
        Res.Send<TJSONObject>(TResponseUtils.InternalError('Ocorreu um erro interno ao processar a solicitacao', LCorrelationId)).Status(THTTPStatus.InternalServerError);
      end;
    end;
  finally
...
begin
  ...
  try
    try
      ...
      begin
        LCorrelationId := ObterCorrelationId(Req);
        TLogger.Error(Format('[%s] %s', [LCorrelationId, 'ConsultaController.ObterAnamnese']), E);
        Res.Send<TJSONObject>(TResponseUtils.InternalError('Ocorreu um erro interno ao processar a solicitacao', LCorrelationId)).Status(THTTPStatus.InternalServerError);
      end;
    end;
  finally
...
begin
  ...
  try
    try
      ...
      TAuditoriaService.Registrar(Req, 'UPDATE', 'ANAMNESE', LId, 'Gravacao de anamnese');
      ...
      begin
        LCorrelationId := ObterCorrelationId(Req);
        TLogger.Error(Format('[%s] %s', [LCorrelationId, 'ConsultaController.CriarAnamnese']), E);
        Res.Send<TJSONObject>(TResponseUtils.InternalError('Ocorreu um erro interno ao processar a solicitacao', LCorrelationId)).Status(THTTPStatus.InternalServerError);
      end;
    end;
  finally
...
begin
  ...
  try
    try
      ...
      begin
        LCorrelationId := ObterCorrelationId(Req);
        TLogger.Error(Format('[%s] %s', [LCorrelationId, 'ConsultaController.AtualizarAnamnese']), E);
        Res.Send<TJSONObject>(TResponseUtils.InternalError('Ocorreu um erro interno ao processar a solicitacao', LCorrelationId)).Status(THTTPStatus.InternalServerError);
      end;
    end;
  finally
...
begin
  ...
  try
    try
      ...
      begin
        LCorrelationId := ObterCorrelationId(Req);
        TLogger.Error(Format('[%s] %s', [LCorrelationId, 'ConsultaController.ExcluirAnamnese']), E);
        Res.Send<TJSONObject>(TResponseUtils.InternalError('Ocorreu um erro interno ao processar a solicitacao', LCorrelationId)).Status(THTTPStatus.InternalServerError);
      end;
    end;
  finally
...
begin
  ...
  try
    try
      ...
      TAuditoriaService.Registrar(Req, 'PRINT', 'CONSULTA', ParamId(Req), 'Impressao de documento clinico');
    except
      on E: EConsultaNaoEncontrada do
        Res.Send<TJSONObject>(TResponseUtils.NotFound(E.Message)).Status(THTTPStatus.NotFound);
      on E: Exception do
      begin
        LCorrelationId := ObterCorrelationId(Req);
        TLogger.Error(Format('[%s] %s', [LCorrelationId, 'ConsultaController.ImpressaoAnamnese']), E);
        Res.Send<TJSONObject>(TResponseUtils.InternalError('Ocorreu um erro interno ao processar a solicitacao', LCorrelationId)).Status(THTTPStatus.InternalServerError);
      end;
    end;
  finally
...
begin
  ...
  try
    try
      ...
      begin
        LCorrelationId := ObterCorrelationId(Req);
        TLogger.Error(Format('[%s] %s', [LCorrelationId, 'ConsultaController.ObterAnamneseConsulta']), E);
        Res.Send<TJSONObject>(TResponseUtils.InternalError('Ocorreu um erro interno ao processar a solicitacao', LCorrelationId)).Status(THTTPStatus.InternalServerError);
      end;
    end;
  finally
...
begin
  ...
  try
    try
      ...
      begin
        LCorrelationId := ObterCorrelationId(Req);
        TLogger.Error(Format('[%s] %s', [LCorrelationId, 'ConsultaController.SalvarAnamneseConsulta']), E);
        Res.Send<TJSONObject>(TResponseUtils.InternalError('Ocorreu um erro interno ao processar a solicitacao', LCorrelationId)).Status(THTTPStatus.InternalServerError);
      end;
    end;
  finally
...
begin
  ...
  try
    try
      ...
      begin
        LCorrelationId := ObterCorrelationId(Req);
        TLogger.Error(Format('[%s] %s', [LCorrelationId, 'ConsultaController.ListarPrescricoes']), E);
        Res.Send<TJSONObject>(TResponseUtils.InternalError('Ocorreu um erro interno ao processar a solicitacao', LCorrelationId)).Status(THTTPStatus.InternalServerError);
      end;
    end;
  finally
...
begin
  ...
  try
    try
      ...
      begin
        LCorrelationId := ObterCorrelationId(Req);
        TLogger.Error(Format('[%s] %s', [LCorrelationId, 'ConsultaController.ObterPrescricao']), E);
        Res.Send<TJSONObject>(TResponseUtils.InternalError('Ocorreu um erro interno ao processar a solicitacao', LCorrelationId)).Status(THTTPStatus.InternalServerError);
      end;
    end;
  finally
...
begin
  ...
  try
    try
      ...
      TAuditoriaService.Registrar(Req, 'UPDATE', 'PRESCRICAO', LId, 'Gravacao de prescricao');
      ...
      begin
        LCorrelationId := ObterCorrelationId(Req);
        TLogger.Error(Format('[%s] %s', [LCorrelationId, 'ConsultaController.CriarPrescricao']), E);
        Res.Send<TJSONObject>(TResponseUtils.InternalError('Ocorreu um erro interno ao processar a solicitacao', LCorrelationId)).Status(THTTPStatus.InternalServerError);
      end;
    end;
  finally
...
var
  Service: TConsultaService;
  LPrescricaoId, LConsultaId: Integer;
  LCorrelationId: string;
begin
  Service := TConsultaService.Create;
  try
    try
      LConsultaId := 0;
      LPrescricaoId := StrToIntDef(Req.Params.Items['prescricao_id'], 0);
      if LPrescricaoId > 0 then
        LConsultaId := ParamId(Req)
      else
        LPrescricaoId := ParamId(Req);

      Service.AtualizarPrescricao(LPrescricaoId, Req.Body<TJSONObject>, LConsultaId);
      TAuditoriaService.Registrar(Req, 'UPDATE', 'PRESCRICAO', LPrescricaoId, 'Gravacao de prescricao');
      SendId(Res, 'Prescricao atualizada com sucesso', 'id', LPrescricaoId, THTTPStatus.OK);
    except
      on E: EConsultaNaoEncontrada do
        Res.Send<TJSONObject>(TResponseUtils.NotFound(E.Message)).Status(THTTPStatus.NotFound);
      on E: EConsultaValidacao do
      ...
      begin
        LCorrelationId := ObterCorrelationId(Req);
        TLogger.Error(Format('[%s] %s', [LCorrelationId, 'ConsultaController.AtualizarPrescricao']), E);
        Res.Send<TJSONObject>(TResponseUtils.InternalError('Ocorreu um erro interno ao processar a solicitacao', LCorrelationId)).Status(THTTPStatus.InternalServerError);
      end;
    end;
  finally
...
begin
  ...
  try
    try
      ...
      Service.ExcluirPrescricao(LPrescricaoId, LConsultaId);
      TAuditoriaService.Registrar(Req, 'DELETE', 'PRESCRICAO', LPrescricaoId, 'Exclusao de prescricao');
      ...
      begin
        LCorrelationId := ObterCorrelationId(Req);
        TLogger.Error(Format('[%s] %s', [LCorrelationId, 'ConsultaController.ExcluirPrescricao']), E);
        Res.Send<TJSONObject>(TResponseUtils.InternalError('Ocorreu um erro interno ao processar a solicitacao', LCorrelationId)).Status(THTTPStatus.InternalServerError);
      end;
    end;
  finally
...
begin
  ...
  try
    try
      ...
      begin
        LCorrelationId := ObterCorrelationId(Req);
        TLogger.Error(Format('[%s] %s', [LCorrelationId, 'ConsultaController.ImpressaoPrescricao']), E);
        Res.Send<TJSONObject>(TResponseUtils.InternalError('Ocorreu um erro interno ao processar a solicitacao', LCorrelationId)).Status(THTTPStatus.InternalServerError);
      end;
    end;
  finally
...
begin
  ...
  try
    try
      ...
      begin
        LCorrelationId := ObterCorrelationId(Req);
        TLogger.Error(Format('[%s] %s', [LCorrelationId, 'ConsultaController.ListarDocumentos']), E);
        Res.Send<TJSONObject>(TResponseUtils.InternalError('Ocorreu um erro interno ao processar a solicitacao', LCorrelationId)).Status(THTTPStatus.InternalServerError);
      end;
    end;
  finally
...
begin
  ...
  try
    try
      ...
      begin
        LCorrelationId := ObterCorrelationId(Req);
        TLogger.Error(Format('[%s] %s', [LCorrelationId, 'ConsultaController.CriarDocumento']), E);
        Res.Send<TJSONObject>(TResponseUtils.InternalError('Ocorreu um erro interno ao processar a solicitacao', LCorrelationId)).Status(THTTPStatus.InternalServerError);
      end;
    end;
  finally
...
begin
  ...
  try
    try
      ...
      begin
        LCorrelationId := ObterCorrelationId(Req);
        TLogger.Error(Format('[%s] %s', [LCorrelationId, 'ConsultaController.ObterDocumento']), E);
        Res.Send<TJSONObject>(TResponseUtils.InternalError('Ocorreu um erro interno ao processar a solicitacao', LCorrelationId)).Status(THTTPStatus.InternalServerError);
      end;
    end;
  finally
...
begin
  ...
  try
    try
      ...
      begin
        LCorrelationId := ObterCorrelationId(Req);
        TLogger.Error(Format('[%s] %s', [LCorrelationId, 'ConsultaController.AtualizarDocumento']), E);
        Res.Send<TJSONObject>(TResponseUtils.InternalError('Ocorreu um erro interno ao processar a solicitacao', LCorrelationId)).Status(THTTPStatus.InternalServerError);
      end;
    end;
  finally
...
begin
  ...
  try
    try
      ...
      TAuditoriaService.Registrar(Req, 'PRINT', 'CONSULTA', LId, 'Impressao de documento clinico');
      ...
      begin
        LCorrelationId := ObterCorrelationId(Req);
        TLogger.Error(Format('[%s] %s', [LCorrelationId, 'ConsultaController.EmitirDocumento']), E);
        Res.Send<TJSONObject>(TResponseUtils.InternalError('Ocorreu um erro interno ao processar a solicitacao', LCorrelationId)).Status(THTTPStatus.InternalServerError);
      end;
    end;
  finally
...
begin
  ...
  try
    try
      ...
      begin
        LCorrelationId := ObterCorrelationId(Req);
        TLogger.Error(Format('[%s] %s', [LCorrelationId, 'ConsultaController.ImpressaoDocumento']), E);
        Res.Send<TJSONObject>(TResponseUtils.InternalError('Ocorreu um erro interno ao processar a solicitacao', LCorrelationId)).Status(THTTPStatus.InternalServerError);
      end;
    end;
  finally
...
begin
  ...
  try
    try
      ...
      begin
        LCorrelationId := ObterCorrelationId(Req);
        TLogger.Error(Format('[%s] %s', [LCorrelationId, 'ConsultaController.ListarAnexos']), E);
        Res.Send<TJSONObject>(TResponseUtils.InternalError('Ocorreu um erro interno ao processar a solicitacao', LCorrelationId)).Status(THTTPStatus.InternalServerError);
      end;
    end;
  finally
...
begin
  ...
  try
    try
      ...
      begin
        LCorrelationId := ObterCorrelationId(Req);
        TLogger.Error(Format('[%s] %s', [LCorrelationId, 'ConsultaController.CriarAnexo']), E);
        Res.Send<TJSONObject>(TResponseUtils.InternalError('Ocorreu um erro interno ao processar a solicitacao', LCorrelationId)).Status(THTTPStatus.InternalServerError);
      end;
    end;
  finally
